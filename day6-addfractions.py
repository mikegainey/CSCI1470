...
#   It may not be the lowest common denominator, so the result, while correct, may not be
#   in simplest form.
cd = d1 * d2 # for 4/5 + 3/4, the cd = 5 * 4 = 20

# adjust the numerators of the two numbers to reflect the new, common denominator
n1cd = (cd / d1) * n1 # (20 / 5) * 4 = 16, so 4/5 becomes 16/20
n2cd = (cd / d2) * n2 # (20 / 4) * 3 = 15, so 3/4 becomes 15/20

# rn and rd are the result numerator and result denominator
rn = n1cd + n2cd # add the adjusted numerators
rd = cd          # the denominator of the result (rd) is the common denominator

rn = int(rn) # division results in a float; get rid of the .0
